...
Resonova needs a voice cloning / TTS model that:
...
| Suitability for Resonova | ❌ Hindi support is insufficient for the primary use case |
...
| Open-weight | ✅ MIT licensed |
| Suitability for Resonova | ❌ Eliminated due to no Hindi support |
...
| Suitability for Resonova | Viable alternative but less documented for Hindi than XTTS-v2 |
...
1. **Hindi is non-negotiable**: Resonova's primary output language is Hindi. XTTS-v2 is the most tested open-weight zero-shot TTS model with documented Hindi support. YourTTS and Bark are eliminated outright.
...
3. **Zero-shot cloning from a short clip**: XTTS-v2 requires only 6+ seconds of reference audio with no fine-tuning — this matches Resonova's design goal of working with any speaker's recording without training.
...
- Resonova pins the working version (`TTS==0.22.0`) to prevent silent breakage from future changes
