...
Resonova needs a translation model that:
...
1. **Domain fit**: Resonova's primary use case is English → Hindi for media dubbing. IndicTrans2 was trained and benchmarked specifically on Indic language pairs — this is exactly its design goal, not a side use case.
...
3. **Extensibility**: IndicTrans2 covers all 22 Indian official languages. If Resonova later adds Tamil, Bengali, or other targets, the same model handles it without a model swap.
