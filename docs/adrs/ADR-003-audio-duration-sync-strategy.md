...
**Context:** Project Resonova — AI dubbing pipeline (English → Hindi)
...
- **Implementation**: Written using an `ffmpeg` subprocess in `resonova/pipeline.py` with custom logic to chain `atempo` filters (since a single `atempo` filter in ffmpeg only supports ranges between 0.5 and 2.0).
